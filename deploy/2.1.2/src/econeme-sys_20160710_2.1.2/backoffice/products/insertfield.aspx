<%@ Page Language="C#" AutoEventWireup="true" CodeFile="insertfield.aspx.cs" Inherits="_ProductField" Debug="false" ValidateRequest="false"%>
...
function insertField(){
	...
	document.form_inserisci.list_product_fields_values.value = "";	
	document.form_inserisci.list_product_fields_ml_values.value = "";	
		...
		$("input:text[id*='field_values_']").each(function(){
			if($(this).attr('id').indexOf("_ml")<0){
				if($(this).val()==''){
					...
					document.form_inserisci.list_product_fields_values.value +=$(this).val()+"##";
				}
			}else{
				if($(this).val()!=''){
					...
					document.form_inserisci.list_product_fields_ml_values.value +=currvalue+"|"+lang+"="+$(this).val()+"##";
				}				
			}
		});	
	...
	document.form_inserisci.list_product_fields_values.value = document.form_inserisci.list_product_fields_values.value.substring(0,document.form_inserisci.list_product_fields_values.value.lastIndexOf("##"));
	document.form_inserisci.list_product_fields_ml_values.value = document.form_inserisci.list_product_fields_ml_values.value.substring(0,document.form_inserisci.list_product_fields_ml_values.value.lastIndexOf("##"));
	//alert(document.form_inserisci.list_product_fields_ml_values.value);
...
<div id="backend-warp">
	...
	<div id="container">
		...
		<form action="/backoffice/products/insertfield.aspx" method="post" name="form_inserisci">
		  ...
		  <input type="hidden" value="" name="list_product_fields_ml_values">
		...
		<td>	
			<div align="left" style="float:top;padding-right:60px;">
				...
				<%foreach (Language x in languages){%>
				<input type="text" hspace="2" vspace="2" name="description_<%=x.label%>" id="description_<%=x.label%>" value="<%=mlangrep.translate("backend.prodotti.detail.table.label.description_"+field.description, x.label, lang.defaultLangCode)%>" class="formFieldTXTInternationalization">
		  	...
		  	<div align="left" style="float:top;padding-top:0px;height:40px;">			
				...
				<input type="text" name="id_group" id="id_group"  value="<%=field.groupDescription%>" class="formFieldTXTMedium2" onkeypress="javascript:return notSpecialCharButUnderscore(event);">
				...
				<%foreach (Language x in languages){%>
				<input type="text" hspace="2" vspace="2" name="id_group_<%=x.label%>" id="id_group_<%=x.label%>" value="<%=mlangrep.translate("backend.prodotti.detail.table.label.id_group_"+field.groupDescription, x.label, lang.defaultLangCode)%>" class="formFieldTXTInternationalization">
				&nbsp;<img width="16" height="11" border="0" style="padding-left:5px;padding-right:5px;vertical-align:middle;" alt="<%=lang.getTranslated("portal.header.label.desc_lang."+x.label)%>" title="<%=lang.getTranslated("portal.header.label.desc_lang."+x.label)%>" src="/backoffice/img/flag/flag-<%=x.label%>.png"><br/>
				<%}%>					
				</div>			
		  	</div><br> 
		  ...
		  <div align="left" id="field_values_div">
			...
			<%if(fieldValues.Count>0){
				int counter = 0;
				foreach(ProductFieldsValue ufv in fieldValues){%>
					<span id="field_values_container<%=counter%>">
						<input type="text" name="field_values_<%=counter%>" id="field_values_<%=counter%>" value="<%=ufv.value%>" class="formFieldTXT" onkeypress="javascript:return notSpecialCharButUnderscore(event);">
						...
						<%foreach (Language x in languages){%>
						<input type="text" hspace="2" vspace="2" name="field_values_ml_<%=counter%>_<%=x.label%>" id="field_values_ml_<%=counter%>_<%=x.label%>" value="<%=mlangrep.translate("backend.prodotti.detail.table.label.field_values_"+field.description+"_"+ufv.value, x.label, lang.defaultLangCode)%>" class="formFieldTXTInternationalization">
						&nbsp;<img width="16" height="11" border="0" style="padding-left:5px;padding-right:5px;vertical-align:middle;" alt="<%=lang.getTranslated("portal.header.label.desc_lang."+x.label)%>" title="<%=lang.getTranslated("portal.header.label.desc_lang."+x.label)%>" src="/backoffice/img/flag/flag-<%=x.label%>.png"><br/>
						<%}%>					
						</div>
					</span>					
				...
				<span id="field_values_container0">
					<input type="text" name="field_values_0" id="field_values_0" value="" class="formFieldTXT" onkeypress="javascript:return notSpecialCharButUnderscore(event);">
