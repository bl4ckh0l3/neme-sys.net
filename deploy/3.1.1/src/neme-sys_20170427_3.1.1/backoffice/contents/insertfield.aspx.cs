...
public partial class _ContentField : Page 
{
	...
	protected ContentField field;
	protected IList<ContentFieldsValue> fieldValues;
	...
	protected void Page_Load(Object sender, EventArgs e)
	{
		...
		fieldValues = new List<ContentFieldsValue>();
		...
		{	
			try{
				field = contentrep.getContentFieldById(Convert.ToInt32(Request["id"]));
				fieldValues = contentrep.getContentFieldValues(field.id);
			}catch (Exception ex){
				field = new ContentField();		
				field.id = -1;	
				fieldValues = new List<ContentFieldsValue>();
			...
			try
			{
				...
				if(languages!=null){
					foreach (Language x in languages){
						//*** insert description
						ml = mlangrep.find("backend.contenuti.detail.table.label.description_"+field.description, x.label);
						if(ml != null){
							...
							ml.keyword = "backend.contenuti.detail.table.label.description_"+field.description;
							ml.langCode = x.label;
							ml.value = Request["description_"+x.label];
							if(!String.IsNullOrEmpty(ml.value)){					
						...
						ml = mlangrep.find("backend.contenuti.detail.table.label.id_group_"+field.groupDescription, x.label);
						if(ml != null){
							...
							ml = new MultiLanguage();
							ml.keyword = "backend.contenuti.detail.table.label.id_group_"+field.groupDescription;
							ml.langCode = x.label;
							ml.value = Request["id_group_"+x.label];
							if(!String.IsNullOrEmpty(ml.value)){					
								newtranslactions.Add(ml);
				...
				{
					if(!String.IsNullOrEmpty(Request["list_content_fields_values"]))
					{
						string[] pfv = Regex.Split(Request["list_content_fields_values"],"##");							
						if(pfv!=null){
							int counter = 1;
							foreach (string item in pfv){
								ContentFieldsValue ufv = new ContentFieldsValue();
					...
					if(!String.IsNullOrEmpty(Request["list_content_fields_ml_values"]))
					{
						string[] pfv = Regex.Split(Request["list_content_fields_ml_values"],"##");							
						if(pfv!=null){							
							foreach (string item in pfv){
								...
								ml = mlangrep.find("backend.contenuti.detail.table.label.field_values_"+field.description+"_"+originalValue, langV);
								if(ml != null){
									ml.value = currValue;	
									if(!String.IsNullOrEmpty(ml.value)){
										updtranslactions.Add(ml);
									...
									ml = new MultiLanguage();
									ml.keyword = "backend.contenuti.detail.table.label.field_values_"+field.description+"_"+originalValue;
									ml.langCode = langV;
									ml.value = currValue;
									if(!String.IsNullOrEmpty(ml.value)){					
										newtranslactions.Add(ml);
				...
				try
				{
					contentrep.saveCompleteContentField(field, newFieldValues, newtranslactions, updtranslactions, deltranslactions);
					...
					log.msg = "save content field: "+field.ToString();
					log.type = "info";
