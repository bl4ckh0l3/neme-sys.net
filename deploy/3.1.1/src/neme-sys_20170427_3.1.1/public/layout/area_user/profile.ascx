...
<script runat="server">
	...
	{	
		...
		StringBuilder happyUrl = new StringBuilder(secureURL).Append("area_user/profile.aspx");		
...
function deleteStatus(id_status){
	...
	$.ajax({
		type: "POST",
		cache: false,
		url: "<%=secureURL%>area_user/ajaxdeletepreference.aspx",
		data: query_string,
		success: function(response) {
			//alert("response: "+response);
			if(response!=1){
		...
		<div id="backend-content">		
			...
			<div id="profilo-utente" style="margin-top:20px;">      
				...
				<div id="send-status" style="display:none;margin-bottom:10px;vertical-align:top;text-align:left;font-size: 10px;text-decoration: none;border:none;padding-left:0px;padding-right:0px;padding-bottom:5px;background:#FFFFFF;">
					<form action="<%=secureURL%>area_user/profile.aspx" method="post" name="form_update_status" accept-charset="UTF-8" enctype="multipart/form-data">		
