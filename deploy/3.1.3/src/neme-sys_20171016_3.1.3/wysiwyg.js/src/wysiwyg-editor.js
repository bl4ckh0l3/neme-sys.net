(function(window, document, $, undefined){
                                  ...
                                  on_keydown, on_keypress, on_keyup, on_autocomplete )
    {
        ...
        {
            // Add image to editor
            var insert_image_wysiwyg = function( url, filename, txtWidth, txtHeight, txtBorder, cmbAlign, txtHSpace, txtVSpace )
            {
		...
		if(txtWidth){$image.attr('width', txtWidth);}
		if(txtHeight){$image.attr('height', txtHeight);}
		if(txtBorder){$image.attr('border', txtBorder);}
		if(cmbAlign){$image.attr('align', cmbAlign);}
		if(txtHSpace){$image.attr('hspace', txtHSpace);}
		if(txtVSpace){$image.attr('vspace', txtVSpace);}
            ...
            if( ! force_imageupload && window.File && window.FileReader && window.FileList )
            {
                ...
                var loadImageFromFile = function( file )
                {
                    ...
                    reader.onload = function(event) {
                        ...
                        insert_image_wysiwyg( dataurl, file.name, $('#txtWidth').val() , $('#txtHeight').val() , $('#txtBorder').val() , $('#cmbAlign').val() , $('#txtHSpace').val() , $('#txtVSpace').val() );
                    };
            ...
            {		    
                ...
                var $input = $fileuploader_input
					 ...
					 var filename = event.target.files[0].name; // FileList object
					
					on_imageupload.call( this,  insert_image_wysiwyg( $('#pathwysiwyg').val()+filename, filename, $('#txtWidth').val() , $('#txtHeight').val() , $('#txtBorder').val() , $('#cmbAlign').val() , $('#txtHSpace').val() , $('#txtVSpace').val() ));
                                    });
                $fileuploader = $('<form/>').attr('id','formuploader').append($input);
            }
            if( $fileuploader )
	    ...
	    // Add image attributes
	     $('<div/>').attr( 'style' ,'padding-bottom:10px')
		.append('<input type="text" size="3" id="txtWidth">')
		.append('&nbsp;Width&nbsp;&nbsp;')
		.append('<input type="text" size="3" id="txtHeight">')
		.append('&nbsp;Height<br/>')
		.append('<input type="text" size="2" id="txtBorder">')
		.append('&nbsp;Border&nbsp;&nbsp;')
		.append('<select id="cmbAlign"><option value="" selected="selected"></option><option value="left">Left</option><option value="absBottom">Abs Bottom</option><option value="absMiddle">Abs Middle</option><option value="baseline">Baseline</option><option value="bottom">Bottom</option><option value="middle">Middle</option><option value="right">Right</option><option value="textTop">Text Top</option><option value="top">Top</option></select>')
		.append('&nbsp;Align<br/>')
		.append('<input type="text" size="2" id="txtHSpace">')
		.append('&nbsp;HSpace&nbsp;&nbsp;')
		.append('<input type="text" size="2" id="txtVSpace">')
		.append('&nbsp;VSpace')
	     .appendTo( $content );
        ...
        {
            // Add video to editor
            var insert_html_wysiwyg = function(html )
            {
                html = $.trim(html||'');
                wysiwygeditor.insertHTML( html ).closePopup().collapseSelection();
            };
            ...
            var $content = $('<div/>').addClass('wysiwyg-toolbar-form')
                                      .attr('unselectable','on');
            // Add video via '<embed/>'
            var $textareaembed = $('<textarea>').addClass('wysiwyg-input wysiwyg-inputtextarea2');
            var $button = toolbar_button( toolbar_submit );
            var $okaybutton = $button.click(function(event){
                                    insert_html_wysiwyg($textareaembed.val() );
                                    event.stopPropagation();
                                    event.preventDefault();
                                    return false;
                                });
