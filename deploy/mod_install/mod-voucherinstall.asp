...
installFormPageVar = publicInstallDirVar & "moduleinstall.asp"
installResultPageVar = "/installed.asp"
globalQueryFile = publicInstallDirVar&"module_install_query.sql"
...
if(request("apply") = "true") then	
	...
	'objDict.add "homepage_bsea","" 
	'objDict.add "contatti_bsea","" 
	'objDict.add "content_bsea","" 
	'objDict.add "search","" 
	'objDict.add "newsletter","" 
	'objDict.add "findus_bsea",""
	'objDict.add "services_bsea",""
	...
	if (objFSO.FolderExists(installDirVar)) then
		call objFSO.DeleteFile(installDirVar&"\moduleinstall.asp",true)
		call objFSO.DeleteFile(installDirVar&"\module_install_query.sql",true)
	end if	
	
	Set objFSO = nothing
	
	If Err.Number<>0 then
		response.write("page moduleinstall error: "&Err.description&"<br/><br/>")
	else			
