...
module_params=request("module_params")
...
<div id="warp">
	<div id="container">	
		...
		<b><%=lang.getTranslated("backend.voucher.mail.label.module_data")%></b><br><br>
		
		<%		
		arrParams = Split(module_params, "***", -1, 1)		
		For y=LBound(arrParams) to UBound(arrParams)
		strPairValue = arrParams(y)
		arrPairValue = Split(strPairValue, "||", -1, 1)%>
		<b><%=arrPairValue(0)%>:</b>&nbsp;<%=arrPairValue(1)%><br><br>
		<%Next%>		
