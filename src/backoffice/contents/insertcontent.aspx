<%@ Page Language="C#" AutoEventWireup="true" CodeFile="insertcontent.aspx.cs" Inherits="_Content" Debug="false" ValidateRequest="false"%>
...
function previewNewsletter(id_newsletter){
	if(id_newsletter != ""){		
		var query_string = "id_newsletter="+id_newsletter+"&id_content="+$('#id').val();	
		//alert(query_string);
		$.ajax({
			async: true,
			type: "GET",
			cache: false,
			url: "/backoffice/contents/ajaxpreviewnewsletter.aspx",
			data: query_string,
			success: function(response) {
				//alert(response);
				$('#previewNewsletterContainer').empty();
				$('#previewNewsletterContainer').append(response);
				$('#previewNewsletterContainer').show();
			},
		...
		alert("<%=lang.getTranslated("backend.contenuti.detail.js.alert.preview_disabled")%>");
	}
}
...
function renderNumImgsTable(counter){
	...
	for(var i=1;i<=counter;i++){
		render=render+'<tr class="attach_table_rows">';
			...
			render=render+'<td><input type="file" id="fileupload'+i+'" name="fileupload'+i+'" class="formFieldTXT"><input type="hidden" value="" name="fileupload_name'+i+'" id="fileupload_name'+i+'">';
			render=render+'<script>';
			render=render+"$('#fileupload"+i+"').blur(function() {";
				render=render+"$('#fileupload_name"+i+"').val($('#fileupload"+i+"').val());";
			render=render+'});';				
			render=render+'<\/script>';		
			render=render+'</td><td>';						
			if(i==1){
	...
	$.ajax({
		...
		type: "GET",
		cache: false,
		url: "/backoffice/contents/ajaxattachlabel.aspx",
		data: query_string,
		success: function(response) {
...
function deleteAttach(id_attach,row, file_path){
	if(confirm("<%=lang.getTranslated("backend.contenuti.detail.js.alert.confirm_del_attach")%>")){		
		...
		$.ajax({
			...
			type: "POST",
			cache: false,
			url: "/backoffice/contents/ajaxdeleteattach.aspx",
			data: query_string,
			success: function(response) {
				//alert(response);	
				$('#'+row).hide();		
...
function deleteField(id_objref, row,refreshrows){
	if(confirm("<%=lang.getTranslated("backend.contenuti.detail.js.alert.confirm_del_field")%>")){			
		...
		$.ajax({
			...
			type: "POST",
			cache: false,
			url: "/backoffice/contents/ajaxdeletefield.aspx",
			data: query_string,
			success: function(response) {
				//alert(response);	
				$('#'+row).remove();	
...
function saveField(counter,row,refreshrow, modify, subcounter){	
	...
	var getout = false;
	$('#inner-table-content-field-list input:text[name="field_description"]').each( function(){	
		if($(this).val() != "" && document.form_create_field.field_description.value == $(this).val() && ($(this).attr('id')!=$('#field_description_'+counter).attr('id')) && $(this).is(':disabled') == false){
			alert("<%=lang.getTranslated("backend.contenuti.detail.js.alert.description_exists")%>");
			$('#field_description_'+counter).focus();
			getout = true;
			return false;											
		}
	...
	$.ajax({
		...
		url: "/backoffice/contents/ajaxsavefield.aspx",
		data: query_string,
		success: function(response) {
...
function previewComments(){	
	var query_string = "id_element="+$('#id').val()+"&element_type=1&mode=insert&container=commentsContainer";	
	//alert(query_string);
...
<div id="backend-warp">
	...
	<div id="container">
		...
		<div id="backend-content">
			...
			<tr> 		  		  
				<td>
					<form action="/backoffice/contents/ajaxsavefield.aspx" method="post" id="form_create_field" name="form_create_field" enctype="multipart/form-data" accept-charset="UTF-8">
					...
					<form action="/backoffice/contents/insertcontent.aspx" method="post" name="form_inserisci" enctype="multipart/form-data" accept-charset="UTF-8">
						...
						if(cf.common){							
							...
							<td id="td_content_field_value_<%=cf.id%>">
							 ...
							 <tr style="display:none;" id="tr_ccontent_field_edit_<%=cf.id%>">
								...
								<input type="hidden" value="<%=cf.id%>" name="id_field" id="id_field_<%=cf.id%>">						 
								 <div style="float:left;padding-right:20px;padding-top:5px;min-width:365px;">	
					...
					<div style="float:left; width:400px;padding-left:90px;">
						...
						<%if(newsletters!= null && newsletters.Count>0){%>	  
							<span class="labelForm"><%=lang.getTranslated("backend.contenuti.detail.table.label.send_as_newsletter")%></span><br>
							<select name="send_newsletter" class="formFieldSelectNewsletter">
							<option value="0" selected><%=lang.getTranslated("backend.commons.no")%></option>
							<option value="1"><%=lang.getTranslated("backend.commons.yes")%></option>
							</select>&nbsp;
							<select name="choose_newsletter" id="choose_newsletter" class="formFieldTXT">
							<option value=""></option>
						...
						<%}%>
						<a href="javascript:previewNewsletter($('#choose_newsletter').val())"><%=lang.getTranslated("backend.contenuti.detail.button.label.preview_newsletter")%></a>  
						<div id="previewNewsletterWrapper" style="position:relative;">
						<div id="previewNewsletterContainer" style="position:absolute;top:-500px;left:-400px;width:600px;min-height:400px;border:1px solid #000;padding:5px;display:none; background-color:#FFFFFF;"></div>
						</div>
						<script>
						$('#previewNewsletterContainer').click(function() {
							$(this).hide();
						});					
						</script>
			...
			<form action="/backoffice/contents/insertcontent.aspx" method="get" name="form_reload_page">
