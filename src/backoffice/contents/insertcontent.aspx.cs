...
public partial class _Content : Page 
{
	...
	protected void Page_Load(Object sender, EventArgs e)
	{
		...
		try{				
			commonfields = contrep.getContentFields(-1, true, null, true);
		...
		{	
			...
			if(carryOn){
				if(savesc==0){
					Response.Redirect("/backoffice/contents/insertcontent.aspx?cssClass="+Request["cssClass"]+"&id="+content.id);
				}else{
					Response.Redirect("/backoffice/contents/contentlist.aspx?cssClass="+Request["cssClass"]);
				}
			}else{
