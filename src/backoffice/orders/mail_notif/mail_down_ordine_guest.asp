			...
			for each z in objSelProdPerOrder
				...
				if(tmpProd.getProdType()=1)then%>
					...
					for each r in objDownProdList
						if not(isNull(objDownProd4Order.getFileByIDProdDown(id_order, tmpProd.getIDProdotto(), r))) then
							...
							if(objDownProd4OrderTmp.isActive())then%>
								&nbsp;&nbsp;<a href="<%=base_url&Application("srt_default_server_name")&Application("baseroot")&"/common/include/DownloadManager.asp?id_ordine="&id_order&"&id_file="&r&"&force=1"%>"><%=objDownProdList(r).getFileName()%></a>&nbsp;&nbsp;<%'=base_url&Application("srt_default_server_name")&Application("baseroot")&"/common/include/DownloadManager.asp?id_ordine="&id_order&"&id_file="&r&"&force=1"%><br/>
							<%end if
