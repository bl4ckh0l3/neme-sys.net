...
Set objDictPaypalNeedField = objPaypal.getListaPaypalFieldMatch()
Set objDictPaypalField = objPaypal.getListaPaypalFieldNotMatch()
...
<%for each y in objPaymentFixedMatchFieldList.Keys
	for each q in objDictPaypalNeedField.Keys
		if(y = objDictPaypalNeedField.item(q)) then%>	
		<div align="left" style="float:top">				
		<input type="text" name="fieldname_<%=q%>" value="<%=objDictPaypalNeedField.item(q)%>" class="formFieldTXTReadOnly" readonly="true">
...
hasValue = false
for each q in objDictPaypalField.Keys
	otherFieldName = q
	otherFieldValue = ""
	...
	if not(hasValue) then
		otherFieldValue = objDictPaypalField(q)
	end if
	%>
	<div align="left" style="float:top">				
...
Set objDictPaypalNeedField = nothing
Set objDictPaypalField = nothing
