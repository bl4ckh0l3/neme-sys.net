...
if not(isEmpty(Session("objCMSUtenteLogged"))) then
	...
	'strCodProd = ConvertFromUTF8(strCodProd)
	strNomeProd = Upload.Form("nome_prod")	
	'strNomeProd = ConvertFromUTF8(strNomeProd)
	...
	Dim targetXprod	
	Dim xTarget
	Dim objDownloadedProdUpload,numMaxProds
	Dim FileUploaded, FileUploadedDown
	Dim uploadsProdDirVar, hasDownAttachment
	...
	if (Cint(id_prod) <> -1) then
		...
		call objLogger.write("process prodotto 2 - modificato prodotto --> id: "&id_prod&"; nome prodotto: "&strNomeProd, objUserLogged.getUserName(), "info")
			...
			for f = 1 to numMaxImgs			
				if(Instr(1, typename(Upload.Files("fileupload"&f)), "IUploadedFile", 1) > 0) then
					...
					if (Instr(1, typename(fileXprod.getFileByFileNameAndIdProd(id_prod,tmpFileName)), "File4ProductsClass", 1) > 0) then
						call fileXprod.modifyFileXProdotto(fileXprod.getFileByFileNameAndIdProd(id_prod,tmpFileName).getFileID(), id_prod, tmpFileName, FileUploaded.ContentType, tmpFilePath, Upload.Form("fileupload"&f & "_dida"), Upload.Form("fileupload"&f & "_label"), objConn)
					else
						call fileXprod.insertFileXProdotto(id_prod, tmpFileName, FileUploaded.ContentType, tmpFilePath, Upload.Form("fileupload"&f & "_dida"), Upload.Form("fileupload"&f & "_label"),objConn)					
					end if
'<!--nsys-demoprdproc6-->					
...
'<!---nsys-demoprdproc6-->
		...
		'/**
		'* se il prodotto e' di tipo download faccio upload sul server con la classe opportuna
		'*/
		if(prod_type=1) then			
			On Error Resume Next	
			Dim tmpDownPath, tmpDownFileXprod
			...
			uploadsProdDirVar = uploadsProdDirVar & "\" & id_prod &"\"
...
'<!---nsys-demoprdproc7-->
			...
			if (ks > 0) then
				...
				for x = 1 to numMaxProds			
					if(Instr(1, typename(Upload.Files("prodfileupload"&x)), "IUploadedFile", 1) > 0) then
						hasDownAttachment = true
						Set FileUploadedDown = Upload.Files("prodfileupload"&x)
						tmpFileName = FileUploadedDown.FileName
						tmpFilePath = id_prod & "/" & tmpFileName
						if (Instr(1, typename(objDownloadedProdUpload.getFileByFileNameAndIdProd(id_prod,tmpFileName)), "DownloadableProductClass", 1) > 0) then
							call objDownloadedProdUpload.modifyDownProd(fileXprod.getFileByFileNameAndIdProd(id_prod,tmpFileName).getFileID(), id_prod, tmpFileName, tmpFilePath, FileUploadedDown.ContentType, FileUploadedDown.Size, objConn)
						else
							call objDownloadedProdUpload.insertDownProd(id_prod, tmpFileName, tmpFilePath, FileUploadedDown.ContentType, FileUploadedDown.Size,objConn)					
						end if
'<!--nsys-demoprdproc8-->						
						FileUploadedDown.SaveAs(uploadsProdDirVar & "\" & FileUploadedDown.Filename)
'<!---nsys-demoprdproc8-->
						Set FileUploadedDown = nothing
					end if
				next
			end if				
		...
		call objLogger.write("process prodotto 2 - inserito prodotto --> id: "&newMaxID&"; nome prodotto: "&strNomeProd, objUserLogged.getUserName(), "info")
		...
		if (ks > 0) then
			...
			for q = 1 to numMaxImgs			
				if(Instr(1, typename(Upload.Files("fileupload"&q)), "IUploadedFile", 1) > 0) then
					...
					call fileXprod.insertFileXProdotto(newMaxID, tmpFileName, FileUploaded.ContentType, tmpFilePath, Upload.Form("fileupload"&q & "_dida"), Upload.Form("fileupload"&q & "_label"),objConn)
...
'<!---nsys-demoprdproc11-->
		...
		'/**
		'* se il prodotto 柤i tipo download faccio upload sul server con la classe opportuna
		'*/
		if(prod_type=1) then
			Set objDownloadedProdUpload = new DownloadableProductClass
			numMaxProds = Upload.Form("numMaxProds")
...
'<!---nsys-demoprdproc12-->	
			...
			if (ks > 0) then
				...
				for y = 1 to numMaxProds			
					if(Instr(1, typename(Upload.Files("prodfileupload"&y)), "IUploadedFile", 1) > 0) then
						hasDownAttachment = true
						Set FileUploadedDown = Upload.Files("prodfileupload"&y)
						tmpFileName = FileUploadedDown.FileName
						tmpFilePath = newMaxID & "/" & tmpFileName
						call objDownloadedProdUpload.insertDownProd(newMaxID, tmpFileName, tmpFilePath, FileUploadedDown.ContentType, FileUploadedDown.Size,objConn)	
'<!--nsys-demoprdproc13-->							
						FileUploadedDown.SaveAs(uploadsProdDirVar & "\" & FileUploadedDown.Filename)
'<!---nsys-demoprdproc13-->	
						Set FileUploadedDown = nothing
