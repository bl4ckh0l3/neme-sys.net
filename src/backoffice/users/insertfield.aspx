<%@ Page Language="C#" AutoEventWireup="true" CodeFile="insertfield.aspx.cs" Inherits="_UserField" Debug="false" ValidateRequest="false"%>
...
function insertField(){
	if(document.form_inserisci.description.value == "") {
		alert("<%=lang.getTranslated("backend.utenti.detail.js.alert.insert_description")%>");
	...
	document.form_inserisci.list_user_fields_values.value = "";	
	document.form_inserisci.list_user_fields_ml_values.value = "";	
		...
		$("input:text[id*='field_values_']").each(function(){
			if($(this).attr('id').indexOf("_ml")<0){
				if($(this).val()==''){
					...
					document.form_inserisci.list_user_fields_values.value +=$(this).val()+"##";
				}
			}else{
				if($(this).val()!=''){
					...
					document.form_inserisci.list_user_fields_ml_values.value +=currvalue+"|"+lang+"="+$(this).val()+"##";
				}				
			}
		});	
		
		if(hasEmpty){
			alert("<%=lang.getTranslated("backend.utenti.detail.js.alert.insert_field_value")%>");
			return false;
		}
	}
	document.form_inserisci.list_user_fields_values.value = document.form_inserisci.list_user_fields_values.value.substring(0,document.form_inserisci.list_user_fields_values.value.lastIndexOf("##"));
	document.form_inserisci.list_user_fields_ml_values.value = document.form_inserisci.list_user_fields_ml_values.value.substring(0,document.form_inserisci.list_user_fields_ml_values.value.lastIndexOf("##"));
	//alert(document.form_inserisci.list_user_fields_ml_values.value);
...
function delFieldValues(counter,id_element, value, field, remove){
	...
	if(remove==1){
		...
		$.ajax({
			type: "POST",
			url: "/backoffice/users/ajaxdeletefieldvalue.aspx",
			data: query_string,
			success: function() {
			$("#"+field).remove();
			}
...
<div id="backend-warp">
	...
	<div id="container">
		...
		<form action="/backoffice/users/insertfield.aspx" method="post" name="form_inserisci">
		  <input type="hidden" value="<%=field.id%>" name="id">
		  <input type="hidden" value="insert" name="operation">
		  <input type="hidden" value="" name="list_user_fields_values">
		  <input type="hidden" value="" name="list_user_fields_ml_values">
		  <input type="hidden" value="<%=Request["cssClass"]%>" name="cssClass">	
		<tr>
		<td>	
			<div align="left" style="float:top;padding-right:60px;">
				<span class="labelForm"><%=lang.getTranslated("backend.utenti.detail.table.label.description")%></span><br>
				<input type="text" name="description" value="<%=field.description%>" class="formFieldTXTMedium2">
				...
				<%foreach (Language x in languages){%>
				<input type="text" hspace="2" vspace="2" name="description_<%=x.label%>" id="description_<%=x.label%>" value="<%=mlangrep.translate("backend.utenti.detail.table.label.description_"+field.description, x.label, lang.defaultLangCode)%>" class="formFieldTXTInternationalization">
		  	...
		  	<div align="left" style="float:top;padding-top:0px;height:40px;">			
				<span class="labelForm"><%=lang.getTranslated("backend.utenti.detail.table.label.group")%></span>
				...
				<br/>
				<div style="visibility:hidden;position:relative;width:236px;background-color: #DEE4E8;border:1px solid #000;padding:2px;" id="id_group_ml">
				<%foreach (Language x in languages){%>
				<input type="text" hspace="2" vspace="2" name="id_group_<%=x.label%>" id="id_group_<%=x.label%>" value="<%=mlangrep.translate("backend.utenti.detail.table.label.id_group_"+field.groupDescription, x.label, lang.defaultLangCode)%>" class="formFieldTXTInternationalization">
				&nbsp;<img width="16" height="11" border="0" style="padding-left:5px;padding-right:5px;vertical-align:middle;" alt="<%=lang.getTranslated("portal.header.label.desc_lang."+x.label)%>" title="<%=lang.getTranslated("portal.header.label.desc_lang."+x.label)%>" src="/backoffice/img/flag/flag-<%=x.label%>.png"><br/>
				<%}%>					
				</div>			
		  	</div><br> 
			<div align="left" style="float:left;padding-right:20px;">	
			<span class="labelForm"><%=lang.getTranslated("backend.utenti.detail.table.label.order")%></span><br>
		  ...
		  <div align="left" style="float:left;padding-right:20px;"><span class="labelForm"><%=lang.getTranslated("backend.utenti.detail.table.label.required")%></span><br>
...
<!--nsys-usrfld1-->
			<option value="2"<%if (2==field.useFor) { Response.Write("selected");}%>><%=lang.getTranslated("backend.utenti.field.use_for.purchase")%></option>	
...
<!---nsys-usrfld1-->
		  ...
		  <br><br>
		  <div align="left" style="float:left;padding-right:20px;">
		  	<span class="labelForm"><%=lang.getTranslated("backend.utenti.detail.table.label.type")%></span><br>
		  ...
		  <div align="left">
		  <span class="labelForm"><%=lang.getTranslated("backend.utenti.detail.table.label.type_content")%></span><br>
		  ...
		  <div align="left" id="field_values_div">
			<span class="labelForm"><%=lang.getTranslated("backend.utenti.detail.table.label.values")%></span>
			...
			<%if(fieldValues.Count>0){
				...
				foreach(UserFieldsValue ufv in fieldValues){%>
					...
					<%foreach (Language x in languages){%>
					<input type="text" hspace="2" vspace="2" name="field_values_ml_<%=counter%>_<%=x.label%>" id="field_values_ml_<%=counter%>_<%=x.label%>" value="<%=mlangrep.translate("backend.utenti.detail.table.label.field_values_"+field.description+"_"+ufv.value, x.label, lang.defaultLangCode)%>" class="formFieldTXTInternationalization">
					&nbsp;<img width="16" height="11" border="0" style="padding-left:5px;padding-right:5px;vertical-align:middle;" alt="<%=lang.getTranslated("portal.header.label.desc_lang."+x.label)%>" title="<%=lang.getTranslated("portal.header.label.desc_lang."+x.label)%>" src="/backoffice/img/flag/flag-<%=x.label%>.png"><br/>
					<%}%>					
					</div>
					</span>					
		  ...
		  <div align="left" id="max_lenght_div">
			  <span class="labelForm"><%=lang.getTranslated("backend.utenti.detail.table.label.max_lenght")%></span><br>
		...
		<br/>
		<input type="button" class="buttonForm" hspace="2" vspace="4" border="0" align="absmiddle" value="<%=lang.getTranslated("backend.utenti.detail.button.inserisci.label")%>" onclick="javascript:insertField();" />&nbsp;&nbsp;<input type="button" class="buttonForm" hspace="2" vspace="4" border="0" align="absmiddle" value="<%=lang.getTranslated("backend.commons.back")%>" onclick="javascript:location.href='/backoffice/users/userlist.aspx?cssClass=LU&showtab=usrfield';" />
		<br/><br/>
		</div>
	</div>
