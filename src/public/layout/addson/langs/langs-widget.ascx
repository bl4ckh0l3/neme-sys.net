...
<ul>
	...
	foreach (Language x in languages){
		...
		if(template != null)
		{
			...
			url = MenuService.resolvePageHrefUrl(urlScheme+"://", 1, x.label, langHasSubDomainActive, langUrlSubdomain, category, template, true);
		...
		<a href="javascript:changeActiveLang('<%=url%>', '<%=x.label%>');" title="<%=lang.getTranslated("portal.header.label.desc_lang."+x.label)%>" class="lang-widget<%if(x.label==lang.currentLangCode){Response.Write("-active");}%>"><img src="/common/img/flag/flag-<%=x.label%>.png" alt="" border="0" align="absmiddle" /></a>
	<%}%>
	</li>
</ul>
