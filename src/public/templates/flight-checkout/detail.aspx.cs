...
public partial class _Detail : Page 
{
	...
	{	
		...
		// start checkout-api integration
		
		if(!String.IsNullOrEmpty(Request["idrequest"]))
		{
			string idRequest = Request["idrequest"];
			string idMac = Request["idmac"];
			
			// start call shopping cart create
			RestClient client = new RestClient();
			client.EndPoint = @"http://checkoutapi.bravofly.com/checkout-api/api/v1/cart";
			client.Method = HttpVerb.POST;
			client.PostData = "{'idMac': '"+idMac+"','idRequest': "+idRequest+",'userInfo': {'ip': '192.168.1.1','uid': null,'userAgent': null},'passengerInformation': {'adults': 1,'infants': 0,'children': 0},'businessProfileId': 'VOLAGRATIS','userCurrency': 'EUR'}";
