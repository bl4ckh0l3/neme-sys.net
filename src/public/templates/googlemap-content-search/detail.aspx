	...
	<div id="container">	
		...
		<div id="content-center">
			...
			<%if (content != null) {%>
				...
				<asp:Literal id="csummary" runat="server" /><br/>
				<asp:Literal id="cdescription" runat="server" />
				
				<%
				if(contentFields.Count>0){%>
					<div style="min-height:20px;"></div>
					<%foreach(ContentField c in contentFields){
						if(c.enabled){
						string labelDesc = ContentService.translate("backend.contenuti.detail.table.label.field_description_"+c.description, c.description, lang.currentLangCode, lang.defaultLangCode);
						
						string currvalue = c.value;
						if(c.type==3){
							if(c.typeContent==7 && !String.IsNullOrEmpty(c.value)){
								string tmpcfval = c.value.Substring(0,c.value.IndexOf('_'));
								foreach(Country ct in countries){
									if(tmpcfval == ct.countryCode){
										currvalue = ct.countryDescription;
										break;
									}
								}
							}else if(c.typeContent==8 && !String.IsNullOrEmpty(c.value)){
								string tmpcfval = c.value.Substring(0,c.value.IndexOf('_'));
						...
						%>
						<div style="min-height:20px;min-width:150px;float:left;font-weight:bold;"><%=labelDesc%></div>
						<div style="min-height:20px;display:block;"><%=currvalue%></div>
						<%}
					}
				}
