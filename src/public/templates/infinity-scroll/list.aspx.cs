...
public partial class _List : Page 
{
	...
	{	
		...
		try
		{			
			contents = contentrep.find(null,null,status,0,null,null,orderBy,matchCategories,matchLanguages,true,true,true,true,this.numPage,itemsXpage,out totalcount);
		...
		if(itemsXpage>0){_totalCPages = (int)totalcount/itemsXpage;}
		if(_totalCPages < 1) {
			_totalCPages = 1;
		}else if(totalcount % itemsXpage != 0 &&  (_totalCPages * itemsXpage) < totalcount) {
