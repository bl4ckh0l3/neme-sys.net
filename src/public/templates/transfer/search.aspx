...
<div id="warp">
	...
	<div id="container">	
		...
		<div id="content-center">
			...
			<form id="searchtransfer" method="post">
			<div class="places-service"></div>
			<h2>Ti aiutiamo a trovare il tuo transfer ideale</h2><br/>
			<div class="card">
				<div class="card-body">
					<div>
						...
						<div class="form-group">
							<div class="form-text">
								<label for="Date" class="">Pickup Date</label>
							</div>
						...
						<br>
						<div class="form-group">
						  <input type="radio" name="returnTrip" id="returnTrip" value="onlyOutbound" checked onclick="$('.form-group-returndate').css('display','none');">&nbsp;Only outbound
						  &nbsp;&nbsp;&nbsp;<input type="radio" name="returnTrip" id="returnTrip" value="return" onclick="$('.form-group-returndate').css('display','block');">&nbsp;Return<br>
						</div>
			
						<div class="form-group-returndate">
							<div class="form-text">
								<label for="Date" class="">Return Date</label>
							</div>
							<div class="location-date-wrapper">
			...
			<script>
			$('.form-group-returndate').css('display','none');
				...
				new PickDropWidget({
					...
					onSubmit: function (pickup_Place, dropoff_Place, event) {
						if (pickup_Place && dropoff_Place) {
						  if ( $('input[name=returnTrip]:checked').val() == "onlyOutbound" ) {
							var url = "<%=formAction%>?" + $.param({
								...
								trip: $('input[name=returnTrip]:checked').val()
							});
						  }
			
						  if ( $('input[name=returnTrip]:checked').val() == "return" ) {
							var url = "<%=formAction%>?" + $.param({
								from_type: 'geo',
								to_type: 'geo',
